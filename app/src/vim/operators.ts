import { type CmSelection, cursorMin, makePos, type Pos } from "@revim/vim-keybindings"
...
export const operators: Record<string, OperatorFunc> = {
  ...
  indentAuto: (adapter, _args, ranges) => {
    adapter.execCommand("indentAuto")
    const vim = adapter.state.vim as VimState
    const res = motions.moveToFirstNonWhiteSpaceCharacter(adapter, ranges[0].anchor, {}, vim, vim.inputState)
    return Array.isArray(res) ? res[0] : res
  },
  changeCase: (adapter, args, ranges, oldAnchor, newHead) => {
  ...
  yank: (adapter, args, ranges, oldAnchor) => {
    const vim = adapter.state.vim as VimState
