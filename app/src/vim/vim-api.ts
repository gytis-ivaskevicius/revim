...
import { type ExCommandFunc, exCommandDispatcher, exCommands } from "./ex-commands"
import { resetVimGlobalState, vimGlobalState } from "./global"
import { exitInsertMode, type InsertModeKey, logKey } from "./insert-mode"
import { _mapCommand, clearInputState, exitVisualMode, mapCommand, maybeInitVimState } from "./keymap_vim"
import { defineMotion, type MotionFunc } from "./motions"
  ...
  noremap(lhs: string, rhs: string, ctx?: Context) {
    ...
    for (let i = actualLength - origLength; i < actualLength && ctxsToMap.length; i++) {
      ...
      if (
        ...
        ctxsToMap = ctxsToMap.filter((el) => mappedCtxs.indexOf(el) === -1)
      }
    }
    ...
    const handleKeyInsertMode = () => {
      ...
      if (match.type === "none") {
        ...
        if (this.lastInsertModeKeyTimer) {
          clearTimeout(this.lastInsertModeKeyTimer)
        }
