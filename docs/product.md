...
- **Fork vim-monaco** — Port vim-monaco's TypeScript vim state machine (motions, operators, keymaps, ex commands) into `packages/app/src/vim/`, strip all browser/Monaco dependencies, and wire to the Rust/ratatui backend via a new `TerminalAdapter` and NAPI-RS FFI surface ([story](stories/005-fork-vim-monaco/story.md))
- **Visual mode** — Render charwise/linewise/blockwise visual selection in the TUI; Ctrl+C now behaves like Esc (cancels operations, exits insert/visual mode) instead of quitting — use `:q` to quit ([story](stories/006-ctrl-c-exit-visual-mode/story.md))
...
- **Terminal resize re-render** — Handle `Event::Resize` from crossterm in the keyboard listener thread, push a synthetic `"Resize"` event through the existing keyboard queue, and re-render the display (viewport, scroll, status bar) in the TypeScript event loop; add `TerminalStatusBar.refresh()` for width-dependent status bar recomputation ([story](stories/025-terminal-resize-rerender/story.md))
- **Status bar flicker fix** — Eliminate status bar flicker during cursor movement by removing redundant `render_frame_internal()` calls from `set_cursor_pos`, `set_selection`, and `set_selections` NAPI functions; make `TerminalStatusBar.update()` read cursor position directly from Rust state via `getCursorPos()` instead of stale cached fields; add `focusEditor()` safety-net render at end of key processing ([story](stories/026-status-bar-flicker-fix/story.md))
- **Tabs bar** — Show a tabs bar at the top of the terminal when two or more buffers are open; each tab displays the buffer number and filename (or `[No Name]`); the active tab is highlighted with reversed style; tabs bar is hidden when only one buffer is open; viewport height adjusts by one row when tabs bar is visible; `just dev` with no arguments opens 2 demo buffers so the tabs bar is always visible during development ([story](stories/027-tabs-bar/story.md))
