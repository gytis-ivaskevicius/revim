...
- **Extract vim types package** — Create `@revim/vim-keybindings` Bun workspace package, extract generic types (`CmSelection`, `Pos`, `KeyMapEntry`, `StringStream`, etc.) from `app/src/vim/`, and update all imports across the vim layer; `Marker` and `EditorAdapter` remain in `app` for now ([story](stories/021-extract-vim-types-package/story.md))
